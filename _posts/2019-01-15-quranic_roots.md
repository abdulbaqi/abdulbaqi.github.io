...
Linux command line is a powerful and free tool. Using just few commands one can productively do powerful text analysis. In this quick tutorial I will demostrate few of such linux commands.
...
So, we know that our file has 128,276 lines of code and 57 lines from top has some copyright notes. Using `tail` I can get a working copy of the file by chopping these few lines from top and display only (128,276 - 57 = 128,219) lines from button and save it into a handy file called `qt`
...
By looking into this structure, here is what each column means. 
...
**Fourth column** contains a number of morphological features seperated by `|`. The one concerning us in this exercise is the feature prefixed by `ROOT:`. See this [documentation](http://corpus.quran.com/documentation/morphologicalfeatures.jsp) for more details of Qruanic morphological freatures.

# `cut` and `sed`

`cut` is cool. It is a handy swiss knife at the hand of a data scientist. It splits the line by a delimeter with option `-d` and then pick whatever coloumn you specify by option `f`. In our case, the fields are seperated by a tab which for which I need to specify `-d'\t\'` as the delimeter, this will throw an error, because `-d` accepts only one character as delimeter but `\t` is two characters. (I found a solution to use `-d$'\t'` but for some reason the Jupyter notebook is not allowing me to use it). If it were just comma, I would have used `-d','` instead.

This gives me an opportunity to introduce another giant called `sed`. Among other things, it will find and replace things in our file. Let us use it to replace those tabs with comma, so cut I can used `cut -d','` later. 
...
So, above we asked to split the Quranic root files by comma `-d','` and wanted to preserve only the fourth column `-f4`, and here we are showing only the `tail`. 
...
And now the powerful `grep`. I will use it to pick the lines that has the `ROOT:` words. I am using *regular expression* hence the `-E` option and want to output only the captured word and not the entire lines by specifying the `-o` option. The patter I used is as follows:

Traverse all lines and return only the lines that has the word ROOT:followed by anything except `|`. 

Regular Expression is a wild beast and worth investing time if you want to analyze text. You can always test using many sites for example [regexr.com](https://regexr.com/)
...
I want to extract the exact roots after the `ROOT:` prefix, and I can employ a `cut` again on the `:` delimeter and take the second column. (I am sure there are better ways).
...
All `sort` does is to alphabatically sort the list, hence all the repeated roots are stacked. What I wanted is a unique list of those roots. Here comes the `uniq` command.
...
The above gives the uniq list of all roots of the Quran without preserving how many of each roots are there.

 Let me use `wc` with the `-l` option to tell me the number of lines
...
**Wow**! with just few commands I managed to discover that the Quran contains a total of **1651** roots.
...
All that is left is to do a `sort` again but with options `-n` to make numeric sort on the counts and `-r` to make reverse sort so in the descending order. Let us take the most popular 20 roots.
...
The most frequent root word in the Quran is (قول) `qwl` which are the derivatives of `saying`. After all the Quran is the sayings of Allah. Anyone would recognize the thousands of times Allah command His prophet Muhammad - peace be upon him - to `say` something and hence it is the greatest prove that the job of Prophet Muhammad is to convey whatever he is told to convey and never to author anything himself. This data analysis just comes to prove that.

**kwn**

Runner's up is all derivatives of (كون) `kwn` which refers to the verb to be. And verb to be is the mother of all actions, and it is the word through which Allah executes His orders. When He intends something, He just says 'Be' and it becomes. Shakespear might had some glimpse of the significance of this verb when he said, "To be or not to be, that is the Question".

**rbb**

The third most frequent word refers to Lord which is another way to say Allah. Actually -with few exceptions- you can add this count to the counts of Allah.

**Amn** and **Elm** These includes all derivatives of faith/believe `Amn` and knowledge `Elm`, showing great emphasis on these two qualities as very essential ingredients for salvation. Islam is nothing but seeking knowledge and having faith accordingly. Also note `Amn` brings in all derivatives of `peace` as well. 

Spend some time studying the significance of each of those roots, but here I focused more on the technical bits.
...
All that is left is to place this entire code inside a shell script and allow the user to run the script passing the surah no. and get the list of roots for that surah.

Open up a file using your favorite editor (I use `mcedit`), and create a file, say `qr.sh` for Quran Roots, as follows:
...
The first line is for the shell to recognize where to find the `bash` shell if needed. The second line is a variable that captures the first argument from user as we will see later. The rest is the exact code we have seen earlier, just note now I am using this variable `$NUM` in the first `grep`.
...
Sure enough, the derivatives of the root word of `rHm` (رحم) is the most frequent word in this surah. 

Just out of curiosity, we know Surah No. 55 (ar-Rahman) has repition of the verse:
