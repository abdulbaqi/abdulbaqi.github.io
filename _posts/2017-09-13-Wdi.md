...
As yo can see, the function returns a list of indicator names and their codes. Let us choose indicator with title `CO2 emissions (kg per 2011 PPP $ of GDP)` and code `EN.ATM.CO2E.PP.GD.KD`
...
There are few points to illustrate in the figure above.

First, note that -in order to render better chart- I have used [`seaborn`](https://seaborn.pydata.org/) data visualization package which basically sits on top of `matplotlib`. 
